...
public class FileValidator {
    ...
    public static void validateFileNotEmpty(String[] headers) throws UserException {
        if (headers == null || headers.length == 0)
            throw new UserException(FILE_IS_EMPTY_EM    );
    }
    ...
    public static void validateValues(String[] lenderRow) throws UserException {
        ...
        if (StringUtils.isEmpty(lenderRow[0]) || StringUtils.isEmpty(lenderRow[1]) || StringUtils.isEmpty(lenderRow[2]))
            throw new UserException(FILE_INCORRECT_LENDER_VALUES_EM);

        float rate = extractValue(lenderRow[0], lenderRow[1], Float::parseFloat);
